...
import (
	...
	"github.com/dgodd/crystal-buildpack/supply"
...
var _ = Describe("Supply", func() {
	...
	Describe("Setup", func() {
		BeforeEach(func() {
			mockManifest.EXPECT().AllDependencyVersions("crystal").Return([]string{"0.21.1", "0.22.3", "0.23.4"})
		})
		...
		Context("user sets CRYSTAL_VERSION", func() {
			BeforeEach(func() {
				Expect(ioutil.WriteFile(filepath.Join(buildDir, "shard.yml"), []byte(`{"name":"appname","crystal":"0.22.x"}`), 0644)).To(Succeed())
			})
	...
	Describe("InstallCrystal", func() {
		BeforeEach(func() {
			...
			mockManifest.EXPECT().InstallDependency(libbuildpack.Dependency{Name: "crystal", Version: "0.23.4"}, gomock.Any()).Do(func(_ interface{}, path string) error {
				os.MkdirAll(filepath.Join(path, "crystal-random-number", "bin"), 0755)
				ioutil.WriteFile(filepath.Join(path, "crystal-random-number", "bin", "binfile"), []byte("bin content"), 0755)
				os.MkdirAll(filepath.Join(path, "crystal-random-number", "lib"), 0755)
				ioutil.WriteFile(filepath.Join(path, "crystal-random-number", "lib", "libfile"), []byte("lib content"), 0644)
				return nil
			})
		})
	...
	Describe("BuildApp", func() {
		Context("installed crystal has src in share/crystal/src", func() {
			BeforeEach(func() {
				supplier.Shard.Name = "appname"
				os.MkdirAll(filepath.Join(depsDir, depsIdx, "crystal-random-number", "share", "crystal", "src"), 0755)
			})
			It("builds the app using src from installed crystal and shards from cachedir", func() {
				mockCommand.EXPECT().Run(gomock.Any()).Do(func(cmd *exec.Cmd) error {
					Expect(cmd.Dir).To(Equal(buildDir))
					Expect(cmd.Env).To(ContainElement(fmt.Sprintf("CRYSTAL_PATH=%s/%s/crystal-random-number/share/crystal/src:%s/shards_lib:src", depsDir, depsIdx, cacheDir)))
					Expect(cmd.Args).To(Equal([]string{
						"crystal", "build", "src/appname.cr", "--release",
		...
		Context("installed crystal has src in src", func() {
			BeforeEach(func() {
				supplier.Shard.Name = "appname"
				os.MkdirAll(filepath.Join(depsDir, depsIdx, "crystal-random-number", "src"), 0755)
			})
			It("builds the app using src from installed crystal and shards from cachedir", func() {
				mockCommand.EXPECT().Run(gomock.Any()).Do(func(cmd *exec.Cmd) error {
					Expect(cmd.Dir).To(Equal(buildDir))
					Expect(cmd.Env).To(ContainElement(fmt.Sprintf("CRYSTAL_PATH=%s/%s/crystal-random-number/src:%s/shards_lib:src", depsDir, depsIdx, cacheDir)))
					Expect(cmd.Args).To(Equal([]string{
						"crystal", "build", "src/appname.cr", "--release",
						"-o", filepath.Join(depsDir, depsIdx, "app"),
					}))
